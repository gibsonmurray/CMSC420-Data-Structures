...
public class Point2D {
	 ...
	 * Squared Euclidean distance to another point.
	 * 
	 * @param pt The other point (possibly null).
	 * @return The squared distance between the two points.
	 */
	public double distanceSq(Point2D pt) {
		if (pt == null) return Double.POSITIVE_INFINITY;
		return Math.pow(coord[0] - pt.coord[0], 2) + Math.pow(coord[1] - pt.coord[1], 2);
	}

	/**
	 * Closer of two points this point, breaking ties lexicographically. If
	 * both points are equal, p1 is returned.
	 * 
	 * @param p1 First candidate (possibly null).
	 * @param p2 Second candidate (possibly null).
	 * @return A reference to the closer point, or lexicographically smaller point, or p1 if identical.
	 */
	public Point2D closerOf(Point2D p1, Point2D p2) {
		if (p1 == null && p2 == null) return null;
		double dist1 = distanceSq(p1);
		double dist2 = distanceSq(p2);
		if      (dist1 < dist2) return p1;
		else if (dist2 < dist1) return p2;
		else { // same distances
			if      (p1.coord[0] < p2.coord[0]) return p1;
			else if (p2.coord[0] < p1.coord[0]) return p2;
			else { // ...and same x-coordinates
				if (p1.coord[1] <= p2.coord[1]) return p1;
				else return p2;
			}
		}
	}
	...
	/**
	 * Compare lexicographically by x then y.
	 * 
	 * @param pt The other point
	 * @return -1 if this is smaller, +1 if this larger, 0 otherwise
	 */
	public int compareTo(Point2D pt) {
		for (int i = 0; i < DIM; i++) {
			if (coord[i] < pt.coord[i]) return -1;
			else if (coord[i] > pt.coord[i]) return +1;
		}
		return 0; // all coordinates match
	}
